...
export default class GameLevel extends Scene {
    ...
    protected cage: Sprite | null = null;
...
updateScene(deltaT: number) {
        ...
        while (this.receiver.hasNextEvent()) {
            ...
            switch (event.type) {
                case HW5_Events.PLAYER_HIT_SWITCH:
                    {
                        ...
                        this.switchLabel.text =
                            "Switches Left: " + (this.totalSwitches - this.switchesPressed);
                ...
                case HW5_Events.PLAYER_HIT_SPECIAL_ITEM:
                    {
                        let node = this.sceneGraph.getNode(event.data.get("node"));
                        let other = this.sceneGraph.getNode(event.data.get("other"));

                        if (node === this.player) {
                            this.handleUnlockKeyCollision(
                                <AnimatedSprite>node,
                                <AnimatedSprite>other
                            );
                        } else {
...
HW5_Events.PLAYER_HIT_SPECIAL_ITEM,
        ]);
    }
    ...
    protected addUI() {
        ...
        let pauseLable = <Label>(
            this.add.uiElement(UIElementType.LABEL, "pauseMenu", {
...
let ContBtn = <Button>this.add.uiElement(
        ...
        ContBtn.onEnter = () => {
            ContBtn.backgroundColor = new Color(255, 255, 255, 0.2);
        };
        ContBtn.onClick = () => {
            this.player.unfreeze();
            this.getLayer("pauseMenu").disable();
        ...
        BackBtn.onEnter = () => {
            BackBtn.backgroundColor = new Color(255, 255, 255, 0.2);
        };
        BackBtn.onClick = () => {
            this.getLayer("pauseMenu").disable();
            this.isPausing = false;
    ...
    protected addKey(spriteKey: string, tilePos: Vec2): void {
        ...
        if (spriteKey === "unlockKey") {
            console.log("special key");
            key.setTrigger("player", HW5_Events.PLAYER_HIT_SPECIAL_ITEM, null);
        } else {
    ...
    protected handleUnlockKeyCollision(
        ...
        if (this.cage) {
            this.cage.destroy();
            this.physicsManager.deregisterObject(this.cage);
            this.cage = null;
        }
    }
}
